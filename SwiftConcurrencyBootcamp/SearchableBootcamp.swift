...
final class SearchableViewModel: ObservableObject {
    ...
    func getSearchSuggestions() -> [String] {
        ...
        if search.contains("pa") {
            suggetions.append("Pasta")
        }
        if search.contains("su") {
            suggetions.append("Sushi")
        }
        if search.contains("bu") {
            suggetions.append("Burger")
        }
        suggetions.append("Market")
        suggetions.append("Grocery")
        
        suggetions.append(CuisineOption.italian.rawValue.capitalized)
        suggetions.append(CuisineOption.japanese.rawValue.capitalized)
        suggetions.append(CuisineOption.american.rawValue.capitalized)
    ...
    func getRestaurantSuggestions() -> [Restaurant] {
        ...
        var suggetions: [Restaurant] = []
        let search = searchText.lowercased()
        if search.contains("ita") {
            suggetions.append(contentsOf: allRestaurants.filter({ $0.cuisine == .italian }))
        }
        if search.contains("jap") {
            suggetions.append(contentsOf: allRestaurants.filter({ $0.cuisine == .japanese }))
        }
